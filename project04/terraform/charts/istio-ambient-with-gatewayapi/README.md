# istio-ambient-with-gatewayapi 테스트 README

Istio **ambient mesh** 기반 워크로드에서 **Gateway API(Gateway → HTTPRoute)** 경유로 서비스까지 정상 통신되는지(end-to-end) 검증한 기록/재현 절차입니다.
...
- 차트 경로: `/home/dongdorrong/github/private/kubernetes/project04/terraform/charts/istio-ambient-with-gatewayapi`
- values
...
kubectl -n demo get httproute demo-istio-ambient-with-gatewayapi -o yaml | rg -n "Accepted|ResolvedRefs|reason|message"
```
...
- `HTTPRoute` 상태: `Accepted=True`, `ResolvedRefs=True`, `ResolvedWaypoints=True`
- 응답 헤더에 `server: istio-envoy` 포함
...
- `HTTPRoute Accepted/ResolvedRefs/ResolvedWaypoints`가 `True True True`
...
- ambient mesh opt-in 라벨이 없으면 워크로드가 ambient로 동작하지 않습니다.
  - 예: `kubectl label ns demo istio.io/dataplane-mode=ambient --overwrite`
