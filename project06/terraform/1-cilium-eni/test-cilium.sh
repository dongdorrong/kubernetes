...
if kubectl -n "$namespace" get ds kube-proxy >/dev/null 2>&1; then
  fail "kube-proxy daemonset should be absent for ENI mode"
fi

if kubectl -n "$namespace" get ds aws-node >/dev/null 2>&1; then
  fail "aws-node daemonset should be absent for ENI mode"
fi
...
[[ "$ipam" == "eni" ]] || fail "ipam expected 'eni' but got '$ipam'"
[[ "$routing_mode" == "native" ]] || fail "routing-mode expected 'native' but got '$routing_mode'"
[[ "$kpr" == "true" ]] || fail "kube-proxy-replacement expected 'true' but got '$kpr'"

info "ENI mode checks passed."
