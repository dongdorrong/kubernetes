...
variable "cluster_pool_ipv4_cidrs" {
  type    = list(string)
  default = ["10.244.0.0/16"]
}

variable "cluster_pool_ipv4_mask_size" {
  type    = number
  default = 24
}
...
locals {
  ...
  cilium_values = {
    ...
    ipam = {
      mode = "cluster-pool"
      operator = {
        clusterPoolIPv4PodCIDRList = var.cluster_pool_ipv4_cidrs
        clusterPoolIPv4MaskSize    = var.cluster_pool_ipv4_mask_size
      }
    }
    routingMode          = "tunnel"
